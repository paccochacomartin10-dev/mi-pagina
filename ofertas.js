...
function addToCart(productId) {
    ...
    if (product) {
        // Usar el sistema global de carrito
        const productData = {
            name: product.name,
...
// === FUNCIÓN: ACTUALIZAR CONTADOR DE CARRITO ===
...
function initMainCountdown() {
    ...
    function updateCountdown() {
        ...
        if (difference <= 0) {
            // Reiniciar contador cuando llegue a 0
            targetDate.setDate(targetDate.getDate() + 7);
            return;
        }
...
function initFlashCountdown() {
    ...
    function updateFlashTimer() {
        if (totalSeconds <= 0) {
            totalSeconds = 24 * 3600; // Reiniciar a 24 horas
        }
...
function initScrollAnimations() {
    ...
    // Observar todas las tarjetas de ofertas
...
function trackOfferClicks() {
    ...
    offerButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            // Aquí puedes agregar tu analytics
            // gtag('event', 'click', { 'offer_name': offerTitle });
        });
    });
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    renderProducts();
    updateCartCount(); // Sincronizar contador del carrito
    initMainCountdown();
    initFlashCountdown();
